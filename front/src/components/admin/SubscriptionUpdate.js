...
const SubscriptionUpdate = ({handleClose,subscItem}) => {
    const [achievement, setAchievement] = useState([]);
    const [bonus, setBonus] = useState([]);
    const [input,setInput] = useState("");

    const [subscription, setSubscription] = useState({
        bonus: subscItem.bonus,
        advantages:subscItem.advantages,
        type: subscItem.type,
        firstMonth:subscItem.firstMonth,
        nextMonths:subscItem.nextMonths
    })
    ...
    return (
        ...
        {
            subscription.bonus.length === 0 || subscription.advantages.length === 0 || subscription.type === "" || subscription.firstMonth === 0 || subscription.nextMonths === 0 ? <Button className="mb-3" variant="danger" onClick={() => alert("Не все инпуты заполнены")}>Изменить</Button>  : <Button className="mb-3" variant="success" onClick={addSubscription}>Изменить</Button>
        }
        </Form>
        </SubscriptionSection>
        </SubscriptionWrapper>
